...
| # | 手法 | 難易度 | ステータス |
|---|------|--------|-----------|
| 1 | モンテカルロ・シミュレーション | ★☆☆☆☆ | ✅ 実装済 |
| 2 | クラスタリング（K-Means / DBSCAN） | ★★☆☆☆ | ✅ 実装済 |
| 3 | 遺伝的アルゴリズム（GA） | ★★★☆☆ | ✅ 実装済 |
...
### フェーズ2: 機能拡張（検討中）
- [ ] **アンサンブル予測** — 3手法の結果を統合して最終予測を出す統合モジュール
- [ ] **データ自動更新** — 当選番号CSVの自動ダウンロード・更新スクリプト
- [ ] **予測履歴管理** — 過去の予測と結果を記録・追跡するデータベース
- [ ] **パラメータチューニング** — 各手法のパラメータ最適化（GA の適応度重みなど）
